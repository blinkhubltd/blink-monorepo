...
export default function PickerIncentivesScreen() {
  const { userId } = useAuth();
  const theme = useTheme();
  const {
    currentUser: user,
    pickerIncentives: dashboard,
    incentivesConfigPicker: config,
    saveUserTargets,
    saveIncentiveConfig,
    pickerBaseEarnings,
  ...
  const handleApplyRecommendedPlan = async (targets: {
    ...
    try {
      await applyRecommendedTargets("PICKER", targets);
  ...
  return (
    ...
    >
      ...
      {!hasActivePlan && (
        ...
        >
          <EmptyState
            ...
            subtitle="Set daily, weekly and monthly targets to track your progress and unlock bonuses."
            action={{
      ...
      <View style={{ flexDirection: "row", gap: 12, marginBottom: 16 }}>
        ...
        {pickerRecommendations && pickerRecommendations.length > 0 && (
          <TouchableOpacity
      ...
      <TargetEditorModal
        ...
        onSave={async (d, w, m) => {
          ...
          setSavingTargets(true);
          try {
            await saveUserTargets("PICKER", d, w, m);
          } finally {
            setSavingTargets(false);
          }
        }}
