...
/**
 * SVGs are imported as React components, not as image assets.
 *
 * Applied to config.transformer BEFORE withNativeWind wraps the config below:
 * withNativeWind installs its own transformer that chains to whatever it finds
 * here, so setting babelTransformerPath afterwards would replace NativeWind's
 * and silently disable every className in the app.
 *
 * react-native-qrcode-svg rides on this, and the QR delivery code is
 * business-critical, so a bundle-time failure here is not cosmetic.
 */
config.transformer = {
  ...config.transformer,
