...
async function getUserEnabledTokens(
  ...
  const tokens: PushTokenDoc[] = await ctx.runQuery(
    api.pushTokens.listUserPushTokens,
    {
      userId,
    },
  );
...
export const notifyRiderAssignment = action({
  ...
  handler: async (
    ...
    // Get rider details
    const rider = await ctx.runQuery(api.users.getRiderById, {
    ...
    // Get order details
    const orderDetails = await ctx.runQuery(api.orders.getOrderById, {
    ...
    const customer = await ctx.runQuery(api.users.getUserById, {
    ...
    const notificationResult = await ctx.runMutation(
      api.userNotifications.createRiderAssignmentNotification,
      {
        riderId: args.riderId,
        orderId: args.orderId,
    ...
    const pushResult = await ctx.runAction(api.notifications.notifyUser, {
      userId: args.riderId,
      type: "delivery",
      title,
      message,
...
export const notifyRiderOrderReady = action({
  ...
  handler: async (
    ...
    const order: any = await ctx.runQuery(api.orders.getOrderById, {
      orderId: args.orderId,
    });
    ...
    // Create feed notification using new specific function
    const notificationResult = await ctx.runMutation(
      api.userNotifications.createOrderReadyNotification,
      {
        riderId: args.riderId,
        orderId: args.orderId,
...
export const notifyVendorPickers = action({
  ...
  handler: async (
    ...
    const orderDetails = await ctx.runQuery(api.orders.getOrderWithItems, {
      orderId: args.orderId,
    });
    if (!orderDetails) {
      console.log("Order not found:", args.orderId);
    ...
    const roles = await ctx.runQuery(api.roles.getAllRoles, {});
    ...
    const allPickers = await ctx.runQuery(api.users.getUsersWithPushTokens, {
    ...
    for (const picker of allPickers) {
      const pickerDetails = await ctx.runQuery(api.users.getUserById, {
        user_id: picker._id,
      });
    ...
    const batchResult = await ctx.runAction(api.notifications.notifyUsers, {
      userIds: pickerIds,
      type: "order_update",
      title,
      message,
...
export const notifyPickerAssignment = action({
  ...
  handler: async (
    ...
    const pickerUser = await ctx.runQuery(api.users.getUserById, {
      user_id: args.pickerId,
    });
    const pickerRole = pickerUser?.role_id
      ? await ctx.runQuery(api.roles.getRole, { id: pickerUser.role_id })
      : null;
    ...
    const result = await ctx.runAction(api.notifications.notifyUser, {
...
export const triggerOrderStatusNotification = action({
  ...
  handler: async (ctx, args) => {
    try {
      const order = await ctx.runQuery(api.orders.getOrderWithItems, {
        orderId: args.orderId,
      });
      ...
      if (
        ...
        if (args.newStatus === "Delivery") {
          ...
          await ctx.runAction(api.notifications.notifyUser, {
          ...
          try {
            const freshOrder = await ctx.runQuery(api.orders.getOrderById, {
              orderId: args.orderId,
            });
            if (freshOrder && freshOrder.payment_mode === "pay_now") {
              ...
              if (needsCode) {
                ...
                try {
                  await ctx.runMutation(api.orders.generateDeliveryCode, {
                    orderId: args.orderId,
                  });
                } catch (genErr) {
              ...
              const codedOrder = await ctx.runQuery(api.orders.getOrderById, {
                orderId: args.orderId,
              });
              if (
...
export const sendDeliveryCode = action({
  ...
  handler: async (
    ...
    try {
      ...
      // Create feed notification using new specific function
      const notificationResult = await ctx.runMutation(
        api.userNotifications.createDeliveryCodeNotification,
        {
          userId: args.userId,
          orderId: args.orderId,
          orderReference: order.reference,
...
export const notifyClearanceDeals = action({
  ...
  handler: async (
    ...
    const allTokens: PushTokenDoc[] = await ctx.runQuery(
      api.pushTokens.listAllEnabledTokens,
    );
    ...
    const result = await ctx.runAction(api.notifications.notifyUsers, {
      userIds,
