...
export const processProductImportJob = action({
  ...
  handler: async (ctx, args) => {
    const job = await ctx.runQuery(internal.importJobs.getJobForAction, {
      id: args.jobId,
    });
    if (!job) throw new Error("Import job not found");

    await ctx.runMutation(internal.importJobs.updateJobProgress, {
      id: args.jobId,
      processed: 0,
      success: 0,
      failed: 0,
    ...
    try {
      ...
      const allCategories = await ctx.runQuery(
        internal.importJobs.getAllCategoriesForAction,
        {},
      );
      ...
      for (let i = 0; i < validProducts.length; i += BATCH_SIZE) {
        ...
        try {
          const result = await ctx.runMutation(
            internal.products.internalBulkCreateProducts,
            { products: batch },
          );
