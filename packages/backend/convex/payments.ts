    ...
    for (const item of cartItems) {
      try {
        ...
        const reservation: Id<"stockReservation"> | Doc<"stockReservation"> =
          await ctx.runMutation(api.stockReservation.reserveStock, {
...
export const preparePaystackSplitForCheckout = action({
  ...
  handler: async (
    ...
    for (const item of args.cartItems) {
      const product = await ctx.runQuery(api.products.getProductsById, {
        id: item.productId,
      });
    ...
    for (const vid of vendorIds) {
      const vendor = await ctx.runQuery(api.vendors.getVendorById, {
        vendorId: vid,
      });
    ...
    const getOrCreatePlatformSubaccount = async (
      ...
      const existing = await ctx.runQuery(api.paystackSubaccounts.getByKey, {
        key,
      });
      ...
      await ctx.runMutation(api.paystackSubaccounts.upsert, {
    ...
    const getOrCreateVendorSubaccount = async (
      ...
      await ctx.runMutation(api.vendors.setVendorPaystackSubaccountCode, {
        vendorId: vendor._id,
        subaccountCode,
      });
      return subaccountCode;
    ...
    const getOrCreateIndustryCommissionSubaccount = async (
      ...
      const industry = await ctx.runQuery(api.industry.getIndustryById, {
        id: industryId,
      });
      ...
      await ctx.runMutation(api.industry.updateIndustry, {
        id: industryId,
        updates: {
          bank_details: {
            business_name,
    ...
    await ctx.runMutation(api.payments.setPaymentSplit, {
      reference: args.reference,
      split_code: splitCode,
      breakdown,
    });
      ...
      try {
        if (linkedOrder && linkedOrder.order_status === "Pending") {
          await ctx.scheduler.runAfter(
            0,
            api.notifications.updateOrderStatusWithNotifications,
            {
              orderId: payment.order_id,
              newStatus: "Confirmed",
            },
...
export const verifyPaystack = action({
  ...
  handler: async (
    ...
    try {
      ...
      await ctx.runMutation(api.payments.applyVerificationResult, {
        reference,
...
export const initiatePaystackTransactionAction = action({
  ...
  handler: async (ctx, args): Promise<InitiatePaystackResult> => {
    ...
    const existing = await ctx.runQuery(api.payments.getPaymentsByOrder, {
      orderId: args.orderId,
    });
    ...
    if (!finalEmail) {
      const customerUser = order.user_id
        ? await ctx.runQuery(api.users.getUserById, { user_id: order.user_id })
        : null;
      finalEmail =
    ...
    const paymentId = await ctx.runMutation(
      api.payments.persistInitiatedPaystackPayment,
      {
        orderId: args.orderId,
...
export const finalizePaidOrders = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      await ctx.runMutation(api.stockReservation.confirmPaymentReservation, {
        orderReference: args.reference,
      });
    ...
    for (const grp of args.orders) {
      ...
      if (approvedPrescription) {
        try {
          await ctx.runMutation(api.pickerAssignment.assignOrderToPicker, {
            orderId,
...
export const finalizePaidClearanceOrders = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (created.length > 1) {
      ...
      try {
        await ctx.runMutation(api.clearanceBatching.createAndDispatchBatch, {
          orderIds,
      ...
      try {
        await ctx.runMutation(api.clearanceBatching.addOrderToBatch, {
