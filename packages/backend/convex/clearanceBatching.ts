...
export const addOrderToBatch = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (existingBatch) {
      ...
      if (updatedOrderIds.length >= maxOrders) {
        await ctx.scheduler.runAfter(
          0,
          internal.clearanceBatching.dispatchBatch,
    ...
    await ctx.scheduler.runAfter(
      waitMinutes * 60 * 1000,
      internal.clearanceBatching.processBatchTimeout,
      { batchId },
    );
...
export const createAndDispatchBatch = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.scheduler.runAfter(0, internal.clearanceBatching.dispatchBatch, {
      batchId,
    });
...
export const processBatchTimeout = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.runMutation(internal.clearanceBatching.dispatchBatch, {
